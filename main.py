...
async def handle_text(message: Message, bot: Bot):
    ...
    try:
        ...
        code_blocks, remaining = extract_code_blocks(full_response)
        if code_blocks:
            await send_code_files(bot, message.chat.id, code_blocks, remaining, reply_to=message.message_id)
        else:
            await send_long_message(bot, message.chat.id, full_response, reply_to=message.message_id)
        if triggered_mood:
...
async def main():
    ...
    dp.message.register(on_photo_message, F.photo)
    dp.message.register(on_animation_message, F.animation)
    dp.message.register(on_sticker_message, F.sticker)
    dp.message.register(on_document_message, F.document)
